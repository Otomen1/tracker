...
export function AndroidCaptureSettings() {
  ...
  useEffect(() => { if (nativeCapture.isNative()) void nativeCapture.getStatus().then(setStatus) }, [])
  if (!nativeCapture.isNative()) return <p className="text-sm text-zinc-500">Automatic bank-notification capture is available in the private Android app.</p>
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between rounded-lg border border-zinc-200 p-3 dark:border-zinc-800"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-emerald-600" /><div><p className="text-sm font-medium">Notification access</p><p className="text-xs text-zinc-500">{status.notificationAccess ? "Enabled" : "Required for automatic capture"}</p></div></div>{!status.notificationAccess && <Button size="sm" onClick={() => void nativeCapture.openNotificationAccess()}>Enable</Button>}</div>
